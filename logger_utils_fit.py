...
from rich.progress import Progress, BarColumn, TextColumn, TimeElapsedColumn, TaskProgressColumn, ProgressColumn
from rich.console import Console
...
def print_efficiency_summary(
    all_pt_bins, data_msg_per_bin, mc_msg_per_bin,
...
def print_fit_summary_rich(
    m, popt, perr, edges_pass, args_bin, BINS_INFO,
    Pearson_chi2, Poisson_chi2, total_ndof,
    args_data, fit_type, sigmoid_eff=False, DATA_NAME=None, MC_NAME=None):
    ...
    # --- Fit Params ---
    param_table = Table(show_lines=True, padding=(0, 1), box=box.ROUNDED, expand=True,
                        header_style=f"bold {COLOR_HIGHLIGHT}")
    ...
    full_param_table = Table(show_lines=True, header_style=f"bold {COLOR_HIGHLIGHT}",
                             padding=(0, 1), box=box.ROUNDED, expand=True)
    ...
    for i, p in enumerate(m.params):
        ...
        full_param_table.add_row(str(i), p.name, f"{p.value:.3f}", err_str,
                                 minos_lower, minos_upper, str(p.is_fixed), low, high)

    panel_table = Panel(full_param_table, title=f"[b {COLOR_PRIMARY}] Full Parameter Details",
                        box=box.ROUNDED, border_style=COLOR_BORDER)
