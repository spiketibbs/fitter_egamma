...
from fitter_plot_model import (
    ...
    logging,
    print_fit_summary_rich,
    print_efficiency_summary,
    CustomTimeElapsedColumn,
    separate_signal_shape
)
from fitter_shapes import x_max, x_min
...
def fit_function(
    ...
    def model_approx_pass(edges, *params):
        result_pass, _ = create_combined_model(fit_type, edges_pass, edges_fail, *params, sigmoid_eff=sigmoid_eff, separate_signal_shape=separate_signal_shape)
        return result_pass

    def model_approx_fail(edges, *params):
        _, result_fail = create_combined_model(fit_type, edges_pass, edges_fail, *params, sigmoid_eff=sigmoid_eff, separate_signal_shape=separate_signal_shape)
        return result_fail

    def model_cdf_pass(edges, *params):
        result_pass, _ = create_combined_model(fit_type, edges_pass, edges_fail, *params, use_cdf=True, sigmoid_eff=sigmoid_eff, separate_signal_shape=separate_signal_shape)
        return result_pass

    def model_cdf_fail(edges, *params):
        _, result_fail = create_combined_model(fit_type, edges_pass, edges_fail, *params, use_cdf=True, sigmoid_eff=sigmoid_eff, separate_signal_shape=separate_signal_shape)
        return result_fail
    ...
    plotter = PassFailPlotter(
        c_pass, errors_pass, c_fail, errors_fail,
        len(edges_pass), edges_pass, edges_fail,
        fit_type, sigmoid_eff=sigmoid_eff, separate_signal_shape=separate_signal_shape
    )
    #print("FIT_CONFIG param names:", FIT_CONFIGS[fit_type]["param_names"])
    #print("Plotter param names:", plotter.param_names)
    ...
    summary_text = print_fit_summary_rich(
        m, popt, perr, edges_pass, args_bin, BINS_INFO, Pearson_chi2, Poisson_chi2, total_ndof, args_data, fit_type, sigmoid_eff=sigmoid_eff, DATA_NAME=data_name, MC_NAME=mc_name
    )
