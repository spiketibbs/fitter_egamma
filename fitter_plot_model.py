...
from fitter_shapes import logging, mass, shape_params, sigmoid, print_efficiency_summary, CustomTimeElapsedColumn, print_fit_summary_rich
import argparse 
import json
import matplotlib.pyplot as plt
...
def get_bin_info(mass):
    if mass == "Z":
        ...
        return {f"bin{i}": (f"pt_{lo}p00To{hi}p00", f"{lo:.2f}-{hi:.2f}") 
            for i, (lo, hi) in enumerate(bin_ranges)
        }
    elif mass == "JPsi_muon":
        return {
...
for sig_name, sig_config in SIGNAL_MODELS.items():
    for bg_name, bg_config in BACKGROUND_MODELS.items():
        ...
        if separate_signal_shape:
            # Add separate signal parameters for pass/fail
...
class PassFailPlotter:
    def __init__(self, cost_func_pass, error_func_pass, cost_func_fail, error_func_fail, n_bins_pass, edges_pass, edges_fail, fit_type, sigmoid_eff=False, separate_signal_shape=False):
        ...
        self.separate_signal_shape = separate_signal_shape
    ...
    def __call__(self, args):
        ...
        if self.separate_signal_shape:
            signal_params_pass = [param_dict[f"{p}_pass"] for p in self.signal_param_names]
            signal_params_fail = [param_dict[f"{p}_fail"] for p in self.signal_param_names]
        else:
            signal_params_pass = [param_dict[p] for p in self.signal_param_names]
            signal_params_fail = signal_params_pass
        ...
        signal_pass = self.signal_func(cx_pass, *signal_params_pass)
        signal_fail = self.signal_func(cx_fail, *signal_params_fail)
...
def create_combined_model(fit_type, edges_pass, edges_fail, *params, use_cdf=False, sigmoid_eff=False, separate_signal_shape):
    ...
    # Shared signal parameters
    sig_name = fit_type.split("_")[0]

    if separate_signal_shape:
        signal_params_pass = [params_dict[f"{p}_pass"] for p in SIGNAL_MODELS[sig_name]["params"]]
        signal_params_fail = [params_dict[f"{p}_fail"] for p in SIGNAL_MODELS[sig_name]["params"]]
    else:
        signal_params_pass = [params_dict[p] for p in SIGNAL_MODELS[sig_name]["params"]]
        signal_params_fail = signal_params_pass
    ...
    for p in BACKGROUND_MODELS[bg_name]["params"]:
        # Try to get the parameter with _pass suffix, if not found, try without suffix
        if f"{p}_pass" in params_dict:
            bg_pass_params.append(params_dict[f"{p}_pass"])
        else:
            bg_pass_params.append(params_dict[p])
            
        if f"{p}_fail" in params_dict:
            bg_fail_params.append(params_dict[f"{p}_fail"])
        else:
            bg_fail_params.append(params_dict[p])

    signal_pass = signal_func(x, *signal_params_pass)
    bg_pass = bg_func(x, *bg_pass_params)
    signal_fail = signal_func(y, *signal_params_fail)
...
# Plot fits
def plot_combined_fit(results, plot_dir=".", data_type="DATA", fixed_params=None, sigmoid_eff=False, args_abseta=None, args_mass=None, separate_signal_shape=False):
    if results is None:
    ...
    # Get signal parameters based on whether we're using separate shapes
    if separate_signal_shape:
        # Separate pass and fail signal parameters
        signal_params_pass = [params[f"{p}_pass"] for p in SIGNAL_MODELS[fit_type.split("_")[0]]["params"]]
        signal_params_fail = [params[f"{p}_fail"] for p in SIGNAL_MODELS[fit_type.split("_")[0]]["params"]]
    else:
        # Shared signal parameters
        signal_params_pass = signal_params_fail = [params[p] for p in SIGNAL_MODELS[fit_type.split("_")[0]]["params"]]
    ...
    signal_pass = (params["N"] - (params["B_p"] + params["B_f"])) * eff * signal_func(x, *signal_params_pass)
    ...
    signal_fail = (params["N"] - (params["B_p"] + params["B_f"])) * (1 - eff) * signal_func(x, *signal_params_fail)
...
def save_fits_and_plots(bin_label, eff_data, eff_data_err, eff_mc, eff_mc_err, data_key, mc_key, outdir="plots"):
    """
    Build a 4-panel summary using existing saved plots.
    Displays:
      - Scale factor ± error on top
      - Data and MC efficiencies ± errors below
      - File names below the plots
    """
    import matplotlib.pyplot as plt
    import matplotlib.image as mpimg
    ...
    if eff_mc > 0:
        sf = eff_data / eff_mc
        sf_err = sf * ((eff_data_err / eff_data)**2 + (eff_mc_err / eff_mc)**2)**0.5
    else:
        sf = 0.0
        sf_err = 0.0
    ...
    # Read images
    img1 = mpimg.imread(data_pass_path)
    img2 = mpimg.imread(data_fail_path)
    img3 = mpimg.imread(mc_pass_path)
    img4 = mpimg.imread(mc_fail_path)

    # Display images
    axes[0,0].imshow(img1); axes[0,0].axis("off")
    axes[1,0].imshow(img2); axes[1,0].axis("off")
    axes[0,1].imshow(img3); axes[0,1].axis("off")
    axes[1,1].imshow(img4); axes[1,1].axis("off")

    # ---- ADD SCALE FACTOR (LARGER FONT) ----
    sf_text = f"Scale Factor = {sf:.3f} ± {sf_err:.3f}"
    fig.text(
        0.5, 0.98, sf_text,
        ha="center", va="bottom",
        fontsize=16,
    ...
    # ---- ADD EFFICIENCIES (SMALLER FONT) ----
    eff_text = f"Data Eff = {eff_data:.3f} ± {eff_data_err:.3f} | MC Eff = {eff_mc:.3f} ± {eff_mc_err:.3f}"
    fig.text(
        0.5, 0.95, eff_text,
        ha="center", va="bottom",
        fontsize=12,
    ...
    # ---- ADD FILE NAMES BELOW ----
    files_text = (
        f"Data Pass: {data_pass_path.name} | Data Fail: {data_fail_path.name}\n"
        f"MC Pass: {mc_pass_path.name} | MC Fail: {mc_fail_path.name}"
    )
    fig.text(
        0.5, 0.05, files_text,
    ...
    outname = outdir_4plot / f"{bin_label}_4plot.png"
