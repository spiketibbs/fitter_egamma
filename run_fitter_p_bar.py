...
def main():
    ...
    with Live(progress_group, refresh_per_second=5, console=console):
        ...
        for pt in args_bin:
            ...
            if data_items and mc_items and config["output"].get("plot_dir"):
                # Take the last pair from this bin (or choose a specific key)
                bin_label = pt
                # For efficiency summary we just use the first (or last) values here
                eff_data = data_eff_list[-1] if data_eff_list else None
                eff_mc   = mc_eff_list[-1] if mc_eff_list else None

                if eff_data is not None and eff_mc is not None:
                    save_fits_and_plots(bin_label, eff_data, data_err, eff_mc, mc_err, data_key, mc_key,
                                        outdir=Path(config["output"]["plot_dir"]))
                    console.print(f"[bold green]4-panel plot saved for {bin_label}")
